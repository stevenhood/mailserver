...
public class TestPop3Server {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < requests.length; i++) {
			for (int j = 0; j < numRequests; j++) {
				...
				System.out.println("response " + i + ": " + issueRequest(request, readers[i], writers[i]));
			}
		}
	...
	public static void setup(int numSockets, int port) throws IOException {
	...
	public static String issueRequest(String request, BufferedReader br, BufferedWriter bw) throws IOException {
		bw.write(request);
		bw.flush();
